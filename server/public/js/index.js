...
var Lottery = function (_mix) {
	...
	function Lottery() {
		...
		_this.name = name; // 区分多个彩种
		_this.cname = cname; // 中文名称
		_this.issue = issue; // 当前期号
		_this.state = state; // 当前状态

		_this.omit = new Map(); //遗漏， Map
		_this.open_code = new Set(); //开奖号码， Set
		_this.open_code_list = new Set(); //开奖记录， Set
		_this.play_list = new Map(); // 玩法列表
		_this.number = new Set(); // 选号，1-11		
		...
		_this.omit_el = '.boll-list li'; // 遗漏选择器
		_this.cur_play = 'r5'; // 当前默认玩法


		_this.initPlayList(); // 初始化各玩法
		_this.initNumber(); // 初始化号码
	...
	_createClass(Lottery, [{
		...
		value: function updateState() {
			...
			this.getState().then(function (res) {
				...
				var open_issue = self.issue.slice(4);
				var today_issue = self.issue.slice(-2);
				var rest = 78 - today_issue * 1;

				var tpl = '\n\t\t\t\t\u4ECA\u5929\u5DF2\u552E' + today_issue + '\u671F\uFF0C\u8FD8\u5269<strong class = "red">' + rest + '</strong>\u671F\n\t\t\t';

				// 更新当前期号
				(0, _jquery2.default)(self.issue_el).text(res.issue);
				(0, _jquery2.default)('#open_issue').text(open_issue);
				(0, _jquery2.default)('.kj-date').html(tpl);
...
var Base = function () {
   ...
   */
		...
		value: function setOmit() {
			...
			try {
				for (var _iterator = self.omit.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			(0, _jquery2.default)(self.omit_el).each(function (index, item) {
				var num = (0, _jquery2.default)(item).find('.btn-boll').text();

				if (self.omit.has(num)) {
					(0, _jquery2.default)(item).find('span').text('1');
				}
			});
		}
   ...
   */
		...
		value: function updateOpenCode(code) {
			var self = this;
			var arr = Array.from(code);

			(0, _jquery2.default)('#open_code_list li').each(function (i, t) {
				(0, _jquery2.default)(t).text(arr[i]);
			});
		}
	}, {
		key: 'updateOpenCodeList',
		value: function updateOpenCodeList(issue, code) {

			var open_issue = issue.slice(4);
			var open_code = Array.from(code);
	...
	_createClass(Timer, [{
		...
		value: function countdown(end, update, handle) {
			...
			if (now - end > 0) {
				...
				var rest_d = Math.floor(last_time / day);

				var last_h = last_time - rest_d * day;
				var rest_h = Math.floor(last_h / hour);

				var last_m = last_h - rest_h * hour;
				var rest_m = Math.floor(last_m / min);

				var last_s = last_m - rest_m * min;
				var rest_s = Math.floor(last_s / sec);
...
var Calculate = function () {
   ...
   */
		...
		value: function computeBonus(active, play_name) {
			...
			var play_num = play[1] * 1; // 当前的玩法数字
			var self = this;

			var arr = new Array(play_num).fill(0);
			var min = void 0,
			    max = void 0;
